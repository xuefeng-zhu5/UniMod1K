...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6, num_fusion_layers=4,
        ...
        if num_encoder_layers == 0:
            ...
            self.encoder_vl = TransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)
    ...
    def forward(self, feat_c, mask_c, pos_embed_c, feat_d, mask_d, pos_embed_d, seq_dict_vl, query_embed, mode="all", return_encoder_output=False):
        """
        ...
        vd_concat = torch.cat((output_encoder_color, output_encoder_depth), 2)
        input_fus = self.neck(vd_concat.permute(1, 2, 0))
        num_tstokens = output_encoder_color.shape[0]
        vd_fusion = self.fusion(input_fus.permute(2, 0, 1), src_key_padding_mask=mask_c, pos=pos_embed_c)   #464
        vl_fusion = self.encoder_vl(seq_dict_vl['feat'], src_key_padding_mask=seq_dict_vl['mask'], pos=seq_dict_vl['pos'])  #504

        vdl_concat = torch.cat((vl_fusion[-num_tstokens:], vd_fusion), 2)
        input_fus_vdl = self.neck(vdl_concat.permute(1, 2, 0))
        memory = self.fusion(input_fus_vdl.permute(2, 0, 1), src_key_padding_mask=mask_c, pos=pos_embed_c)
